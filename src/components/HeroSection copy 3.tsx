...
const services: Service[] = [
  {
    ...
    accentColor: 'purple',
    icon: Server,
    buttonText: 'Miami Dedicated Server Plans'
  },
  {
    id: 'vps',
...
const HeroCarousel: React.FC<HeroCarouselProps> = ({ darkMode }) => {
  ...
  return (
    <section 
      className="relative h-[80vh] overflow-hidden transition-all duration-800 ease-in-out"
    ...
    >
      ...
      {/* Main Content */}
      <div className="container mx-auto px-4 py-16 relative z-10 h-[80vh] flex items-center">
          ...
          <div className="text-center lg:text-left">
            <div className={`transition-all duration-500 ${isTransitioning ? 'opacity-0 transform translate-y-8' : 'opacity-100 transform translate-y-0'}`}>
              <div className="flex items-center justify-center lg:justify-start mb-6">
                <div className={`p-4 rounded-2xl bg-gradient-to-r from-${currentService.accentColor}-400/20 to-${currentService.accentColor}-600/20 backdrop-blur-sm border border-white/10`}>
                  <IconComponent className={`w-12 h-12 text-${currentService.accentColor}-300`} />
                </div>
              </div>
              ...
              <p className="text-lg md:text-xl text-white/90 mb-8 leading-relaxed transition-all duration-700 delay-300 max-w-2xl mx-auto lg:mx-0">
                {currentService.description}
              </p>

              <div className="space-y-4 sm:space-y-0 sm:space-x-4 sm:flex justify-center lg:justify-start transition-all duration-700 delay-500">
                <button className={`w-full sm:w-auto bg-${currentService.accentColor}-600 text-white px-8 py-4 rounded-lg font-semibold hover:bg-${currentService.accentColor}-700 transition-all duration-300 hover:scale-105 hover:shadow-lg hover:shadow-${currentService.accentColor}-500/25 transform`}>
                  {currentService.buttonText}
                </button>
                <button className={`w-full sm:w-auto border border-${currentService.accentColor}-300 text-${currentService.accentColor}-100 px-8 py-4 rounded-lg font-semibold hover:bg-${currentService.accentColor}-700/50 transition-all duration-300 backdrop-blur-sm`}>
                  Learn More
                </button>
              </div>
            </div>
